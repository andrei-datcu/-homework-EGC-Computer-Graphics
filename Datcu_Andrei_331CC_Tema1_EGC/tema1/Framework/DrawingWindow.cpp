...
void DrawingWindow::drawObject(Object2D *o){
	if (o->type == 1){
		glColor3f(o->color.r,o->color.g,o->color.b);
		glBegin(GL_LINES);
		glVertex3f(o->transf_points[0]->x,o->transf_points[0]->y,0);
		glVertex3f(o->transf_points[1]->x,o->transf_points[1]->y,0);
		glEnd();
	}
	else{
		if (o->points.size() >= 3){ //daca are mai mult de 3 puncte
			glColor3f(o->color.r,o->color.g,o->color.b);
			if (!o->fill){
				glBegin(GL_LINE_LOOP); //se poate folosi si GL_LINES, dar trebuie date doua puncte o data
				for (int j = 0; j < (int)o->points.size();j++)
					glVertex3f(o->transf_points[j]->x,o->transf_points[j]->y,0);
				glEnd();
			}
				else{
					glBegin(GL_TRIANGLE_FAN); //se poate folosi si GL_TRIANGLES, dar trebuie sa se dea cate trei puncte o data
...
void DrawingWindow::drawComplexObjects(Visual2D *v){
	for (ComplexObject2D *co : v->visual2D_complexObjects2D){
		for (Object2D o : *co)
			drawObject(&o);
	}
}
...
void DrawingWindow::displayFunction()
{
	...
	if (visuals2D.size() == 0)
		...
		for (int i = 0; i < visuals2D.size(); i++)
		{
			...
			drawComplexObjects(visuals2D[i]);
