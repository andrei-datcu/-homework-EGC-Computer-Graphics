...
Visual2D::Visual2D(float xf1,float yf1,float xf2,float yf2,int xp1,
...
void Visual2D::cadruFereastra(Color color)
{
	...
	Rectangle2D *rectangle = 
		new Rectangle2D(Point2D(XFm,YFm),XFM-XFm,YFM-YFm,color,false);
...
{
	Rectangle2D *rectangle = new Rectangle2D(Point2D(xLog(XPm),yLog(YPm)),
		xLog(XPM)-xLog(XPm),yLog(YPM)-yLog(YPm),color,false);
	DrawingWindow::addObject2D_to_Visual2D(rectangle,this);
}
...
float Visual2D::yLog(int yp)
{
	...
	// GLUT BUG - pentru a se tine cont de dimensiunea marginilor 
	//ferestrei pe Y se scade dimensiunea marginii pe Y
...
void Visual2D::addComplexObject2D(ComplexObject2D *co){
	visual2D_complexObjects2D.push_back(co);
}

void Visual2D::removeComplexObject2D(ComplexObject2D *co){
	for (auto itr = visual2D_complexObjects2D.begin(); 
			itr != visual2D_complexObjects2D.end(); ++itr)
		if (*itr == co){
			visual2D_complexObjects2D.erase(itr);
			break;
		}
}

void Visual2D::addText(Text *t){
	visual2D_texts.push_back(t);
}